...
A taxon may carry an `age`: `{"from": …, "to": …}` in Ma, the fossil range drawn under its deep-time chart. `from` is the older bound of its first appearance and `to` the younger bound of its last occurrence, so both are brackets at the resolution the evidence has, not exact origination and extinction dates; `"to": 0` means living members. A taxon whose endpoints are not settled carries no `age` and its page draws no range.
